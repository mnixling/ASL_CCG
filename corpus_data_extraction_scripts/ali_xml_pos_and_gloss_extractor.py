...
with open(partial_path + r'\ali.xml', 'r') as f_IN:
    ...
    # Lines 44-61 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif '\"darn\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_darn = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_darn)
                else:
        ...
        # Lines 64-89 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif 'anticipation of HUNGRY/WISH' in non_dominant_a_tags.string:
                    non_dominant_hand_HUNGRY_WISH = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_HUNGRY_WISH)
...
"""Code blocks from 93 to 101 take the gloss from both the dominant and non-dominant hand and 
...
# lines 104-112 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 120-162 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
