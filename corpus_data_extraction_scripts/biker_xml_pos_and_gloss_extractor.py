...
with open(partial_path + r'\biker.xml', 'r') as f_IN:
    ...
    # Lines 44-58 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif ('5' in dominant_a_tags.string) and ('CL' not in dominant_a_tags.string) and ('MAN' not in dominant_a_tags.string) and ('CRASH' not in dominant_a_tags.string) and ('ACCIDENT' not in dominant_a_tags.string):
        ...
        # Lines 61-76 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif '(2h)5\"wow\"' in non_dominant_a_tags.string:
                    non_dominant_hand_wow = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_wow)
                else:
...
"""
Lines 107-153 arranges the output for the file master_lexicon.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
