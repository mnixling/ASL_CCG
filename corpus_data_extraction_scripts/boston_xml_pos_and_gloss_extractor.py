...
with open(partial_path + r'\boston-la.xml', 'r') as f_IN:
    ...
    # Lines 44-75 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                # The following elif statements seperate the gloss that does not have an associated POS label
                elif ('5' in dominant_a_tags.string) and ('CL' not in dominant_a_tags.string) and ('WHY' not in dominant_a_tags.string) and ('CALIFORNIA' not in dominant_a_tags.string) and ('STRIPE' not in dominant_a_tags.string):
                    ...
                    dominant_hand_false_start = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_false_start)
                elif 'flat-B\"go ahead\"' in dominant_a_tags.string:
                    dominant_hand_flat_b = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_flat_b)
                elif 'B-L\"go ahead\"' in dominant_a_tags.string:
                    dominant_hand_b_l = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_b_l)
                elif '\"wave\"' in dominant_a_tags.string:
                    dominant_hand_wave = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_wave)
                elif '\"quote\"' in dominant_a_tags.string:
                    dominant_hand_quote = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_quote)
                else:
                    dominant_hand_gloss.append(dominant_a_tags.string)

        for dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '27'}):
            for unwanted_a_tag in dominant_hand_FID_value.find_all('A', {'E': '2834', 'S': '2067', 'VID': '0'}):
        ...
        # Lines 78-107 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif '5\"looking for words\"' in non_dominant_a_tags.string:
                    non_dominant_hand_looking_for_words = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_looking_for_words)
                elif 'G\"focus\"' in non_dominant_a_tags.string:
                    non_dominant_hand_g_focus = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_g_focus)
                elif '5\"I don\'t know\"' in non_dominant_a_tags.string:
                    non_dominant_hand_dont_know = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_dont_know)
                elif '5\"that\'s the way it is\"' in non_dominant_a_tags.string:
                    non_dominant_hand_way_it_is = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_way_it_is)
                elif '\"wave\"' in non_dominant_a_tags.string:
                    non_dominant_hand_wave = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_wave)
                elif '1\"focus\"' in non_dominant_a_tags.string:
                    non_dominant_hand_1_focus = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_1_focus)
                else:
            ...
            for non_dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '29'}):
                for unwanted_a_tag in non_dominant_hand_FID_value.find_all('A', {'E': '100', 'S': '67', 'VID': '18'}):
                    unwanted_a_tag.decompose()
...
"""Code blocks from 111 to 119 take the gloss from both the dominant and non-dominant hand and
...
# lines 122-130 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 138-188 arranges the output for the file master_lexicon.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
