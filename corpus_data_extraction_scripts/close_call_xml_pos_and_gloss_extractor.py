...
with open(partial_path + r'\close call.xml', 'r') as f_IN:
    ...
    # Lines 44-63 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif 'fs-R' in dominant_a_tags.string:
                    dominant_hand_fs_r = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_fs_r)
                else:
        ...
        # Lines 66-80 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif '5\"panic\"' in non_dominant_a_tags.string:
                    non_dominant_5_panic = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_5_panic)
                else:
...
"""Code blocks from 84 to 92 take the gloss from both the dominant and non-dominant hand and
...
# lines 95-103 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 111-165 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
...
#
            ...
            for non_dom_item in non_dominant_hand_GLOSS_POS_id_num_list:
                ...
                if 'Tense + Aspect' in non_dom_item[1]:
                    non_dom_item[1] = re.sub('Tense \+ Aspect', 'Tense_+_Aspect', non_dom_item[1])
                if 'Quantifier + Wh-word' in non_dom_item[1]:
                    non_dom_item[1] = re.sub('Quantifier \+ Wh-word', 'Quantifier_+_Wh-word', non_dom_item[1])
