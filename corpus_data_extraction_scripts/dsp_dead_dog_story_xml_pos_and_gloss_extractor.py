...
with open(partial_path + r'\DSP Dead Dog Story.xml', 'r') as f_IN:
    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif '\"hey\"' in dominant_a_tags.string:
                    dominant_hand_hey = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_hey)
                elif '\"applause\"' in dominant_a_tags.string:
                    dominant_hand_applause = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_applause)
                ...
                elif '\"pray\"' in dominant_a_tags.string:
                    dominant_hand_pray = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_pray)
                elif '\"hands on face\"' in dominant_a_tags.string:
                    dominant_hand_hands_on_face = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_hands_on_face)
                else:
                    dominant_hand_gloss.append(dominant_a_tags.string)

        for dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '28'}):
        ...
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif '\"hand on face\"' in non_dominant_a_tags.string:
                    non_dominant_hand_on_face = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_on_face)
                ...
                elif '\"hand on hip\"' in non_dominant_a_tags.string:
                    non_dominant_hand_on_hip = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_on_hip)
                else:
            ...
            for non_dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '29'}):
                for unwanted_a_tag in non_dominant_hand_FID_value.find_all('A', {'E': '833', 'S': '500', 'VID': '13'}):
...
"""
Lines 128-176 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
