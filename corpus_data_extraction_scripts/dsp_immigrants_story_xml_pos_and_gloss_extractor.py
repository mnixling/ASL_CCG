...
with open(partial_path + r'\DSP Immigrants Story.xml', 'r') as f_IN:
    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif '\"topic change\"' in dominant_a_tags.string:
                    dominant_hand_topic_change = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_topic_change)
                else:
        ...
        # Lines 64-73 is only for non-dominant hand gloss
...
"""Code blocks from 77 to 85 take the gloss from both the dominant and non-dominant hand and
...
# lines 88-96 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 104-154 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
