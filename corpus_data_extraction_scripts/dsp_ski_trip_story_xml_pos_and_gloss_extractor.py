...
with open(partial_path + r'\DSP Ski Trip Story.xml', 'r') as f_IN:
    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif '(2h)\"forget it\"' in dominant_a_tags.string:
                    dominant_hand_forget_it = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_forget_it)
                elif '\"hands up\"' in dominant_a_tags.string:
                    dominant_hand_hands_up = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_hands_up)
                else:
        ...
        # Lines 69-81 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif '\"hands up\"' in non_dominant_a_tags.string:
                    non_dominant_hands_up = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hands_up)
                else:
                    non_dominant_hand_gloss.append(non_dominant_a_tags.string)

            for non_dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '33'}):
...
"""
Lines 112-158 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
