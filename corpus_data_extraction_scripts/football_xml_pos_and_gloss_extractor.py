    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif re.search('fs-L$', dominant_a_tags.string):
                    dominant_fs_L = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_fs_L)
                else:
        ...
        # Lines 67-84 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                if non_dominant_a_tags.has_attr('VID'):
                    ...
                    non_dominant_B_focus = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_B_focus)
                else:
...
"""Code blocks from 88 to 96 take the gloss from both the dominant and non-dominant hand and
...
# lines 99-107 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 115-161 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
