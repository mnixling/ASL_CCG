...
with open(partial_path + r'\lapd.xml', 'r') as f_IN:
    ...
    # Lines 44-66 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif 'you know' in dominant_a_tags.string:
                    dominant_hand_you_know = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_you_know)
                else:
                    dominant_hand_gloss.append(dominant_a_tags.string)

        for dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '27'}):
            for unwanted_a_tag in dominant_hand_FID_value.find_all('A', {'E': '234', 'S': '167', 'VID': '11'}):  # this is an extra tag that should not be there
        ...
        # Lines 69-86 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                if non_dominant_a_tags.has_attr('VID'):
                    ...
                    no_dominant_hand_unlabeled_gloss = non_dominant_a_tags.string
                    ...
                    non_dominant_second_G_focus = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_second_G_focus)
                else:
...
"""Code blocks from 90 to 98 take the gloss from both the dominant and non-dominant hand and
...
# lines 101-109 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 117-163 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
