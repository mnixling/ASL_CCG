...
with open(partial_path + r'\ncslgr10c.xml', 'r') as f_IN:
    ...
    # Lines 44-57 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        ...
        # Lines 60-69 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            ...
            for non_dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '32'}):
...
"""Code blocks from 73 to 81 take the gloss from both the dominant and non-dominant hand and
...
# lines 84-92 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 100-134 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
