...
with open(partial_path + r'\ncslgr10e.xml', 'r') as f_IN:
    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                # The following elif statements seperate the gloss that does not have an associated POS label
                    ...
                    dominant_hand_um = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_um)
                else:
        ...
        # Lines 61-70 is only for non-dominant hand gloss
...
"""Code blocks from 74 to 82 take the gloss from both the dominant and non-dominant hand and
...
# lines 85-93 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 101-143 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
