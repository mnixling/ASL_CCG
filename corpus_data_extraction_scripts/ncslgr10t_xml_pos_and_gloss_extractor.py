    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif '\"count on fingers\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_count_fingers = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_count_fingers)
                elif '\"um\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_um = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_um)
                else:
        ...
        # Lines 64-76 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif '\"count on fingers\"' in non_dominant_a_tags.string:
                    non_dominant_hand_unlabeled_count_fingers = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_unlabeled_count_fingers)
                else:
...
"""Code blocks from 80 to 88 take the gloss from both the dominant and non-dominant hand and
...
# lines 91-99 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 107-157 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
