...
with open(partial_path + r'\roadtrip1.xml', 'r') as f_IN:
    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif '\"it\'s ok\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_ok = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_ok)
                elif 'relief' in dominant_a_tags.string:
                    dominant_hand_unlabeled_relief = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_relief)
                elif 'wave' in dominant_a_tags.string:
                    dominant_hand_unlabeled_wave = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_wave)
                elif 'not give a darn' in dominant_a_tags.string:
                    dominant_hand_unlabeled_not_give_darn = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_not_give_darn)
        ...
        # Lines 76-97 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                if non_dominant_a_tags.has_attr('VID'):
                    ...
                    non_dominant_hand_unlabeled_wave = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_unlabeled_wave)
                elif 'pull over' in non_dominant_a_tags.string:
                    non_dominant_hand_unlabeled_resignation = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_unlabeled_resignation)
...
"""Code blocks from 101 to 109 take the gloss from both the dominant and non-dominant hand and
...
# lines 112-120 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 128-178 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
