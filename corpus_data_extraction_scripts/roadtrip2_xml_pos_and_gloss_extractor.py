...
with open(partial_path + r'\roadtrip2.xml', 'r') as f_IN:
    ...
    # Lines 44-77 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                if dominant_a_tags.has_attr('VID'):
                    y = 0  # this is here to satisfy the if condition
                elif ('5' in dominant_a_tags.string) and ('CL' not in dominant_a_tags.string) and ('WHY' not in dominant_a_tags.string) and ('MAN' not in dominant_a_tags.string) and ('ns' not in dominant_a_tags.string):
                    ...
                    dominant_hand_unlabeled_false_start = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_false_start)
                elif 'L\"well\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_L_well = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_L_well)
                elif '\"oh\"' in dominant_a_tags.string:
                    dominant_hand_oh = dominant_a_tags.string  # this is a GLOSS word that does not have a given POS id number
                    non_dominant_hand_unlabeled_gloss_list.append(dominant_hand_oh)
                elif re.search('B-L"move back"$', dominant_a_tags.string):
                    dominant_hand_unlabeled_B_L_move_back = dominant_a_tags.string  # this is a GLOSS word that does not have a given POS id number
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_B_L_move_back)
                elif re.search('fs-M$', dominant_a_tags.string):
                    dominant_hand_fs_M = dominant_a_tags.string  # this is a GLOSS word that does not have a given POS id number
                    non_dominant_hand_unlabeled_gloss_list.append(dominant_hand_fs_M)
                elif re.search('fs-P$', dominant_a_tags.string):
                    dominant_hand_fs_P = dominant_a_tags.string  # this is a GLOSS word that does not have a given POS id number
                    non_dominant_hand_unlabeled_gloss_list.append(dominant_hand_fs_P)
                else:
                    dominant_hand_gloss.append(dominant_a_tags.string)

        for dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '27'}):
            for unwanted_a_tag in dominant_hand_FID_value.find_all('A', {'E': '1733', 'S': '1700', 'VID': '29'}):
        ...
        # Lines 80-98 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif ('5' in non_dominant_a_tags.string) and ('500' not in non_dominant_a_tags.string):
                ...
                elif 'focus' in non_dominant_a_tags.string:
                    non_dominant_hand_unlabeled_focus = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_unlabeled_focus)
                elif '\"leave\"' in non_dominant_a_tags.string:
                    non_dominant_hand_unlabeled_leave = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_unlabeled_leave)
                else:
                    non_dominant_hand_gloss.append(non_dominant_a_tags.string)

            for non_dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '31'}):
...
"""Code blocks from 102 to 110 take the gloss from both the dominant and non-dominant hand and
...
# lines 113-121 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 129-179 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
    ...
    with open(r'C:\Users\manua\Desktop\NIX_BYU_THESIS\NCSLGR_Corpus_data\only_gloss.txt', 'a') as f_OUT_gloss:
        with open(r'C:\Users\manua\Desktop\NIX_BYU_THESIS\NCSLGR_Corpus_data\only_pos.txt', 'a') as f_OUT_POS:
            ...
            for non_dom_item in non_dominant_hand_GLOSS_POS_id_num_list:
                ...
                if 'Discourse Marker' in non_dom_item[1]:
                    non_dom_item[1] = re.sub('Discourse Marker', 'Discourse_Marker', non_dom_item[1])
