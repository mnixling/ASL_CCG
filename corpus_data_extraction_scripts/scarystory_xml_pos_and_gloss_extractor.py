...
with open(partial_path + r'\scarystory.xml', 'r') as f_IN:
    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif ('5' in dominant_a_tags.string) and ('CL' not in dominant_a_tags.string) and ('MAN' not in dominant_a_tags.string):
                ...
                elif '\"jump up\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_jump_up = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_jump_up)
                else:
        ...
        # Lines 64-81 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            ...
            for non_dominant_hand_FID_value in utterance_tag.find_all('TRACK', {'FID': '29'}):
                for unwanted_non_dominant_a_tags_two in non_dominant_hand_FID_value('A', {'E': '6166', 'S': '6133', 'VID': '15'}):
                    unwanted_non_dominant_a_tags_two.decompose()  # This was an added A tag that was causing misalignment in the POS GLOSS
...
"""Code blocks from 85 to 93 take the gloss from both the dominant and non-dominant hand and
...
# lines 96-104 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 112-160 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
