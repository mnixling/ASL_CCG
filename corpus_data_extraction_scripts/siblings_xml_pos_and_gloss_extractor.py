...
with open(partial_path + r'\siblings.xml', 'r') as f_IN:
    ...
    # Lines 44-67 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif ('5' in dominant_a_tags.string) and ('CL' not in dominant_a_tags.string) and ('MAN' not in dominant_a_tags.string) and ('WHY' not in dominant_a_tags.string):
                ...
                elif 'R\"hoping\"++' in dominant_a_tags.string:
                    dominant_hand_unlabeled_R_hoping = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_R_hoping)
                elif '\"applause\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_applause = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_applause)
                else:
        ...
        # Lines 70-85 is only for non-dominant hand gloss
        for non_dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10001'}):
            for non_dominant_a_tags in non_dominant_track_tags.find_all('A'):
                ...
                elif '5' in non_dominant_a_tags.string:
                ...
                elif '\"hoping\"' in non_dominant_a_tags.string:
                    non_dominant_hand_unlabeled_hoping = non_dominant_a_tags.string
                    non_dominant_hand_unlabeled_gloss_list.append(non_dominant_hand_unlabeled_hoping)
                else:
...
"""Code blocks from 89 to 97 take the gloss from both the dominant and non-dominant hand and
...
# lines 100-108 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 116-166 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
