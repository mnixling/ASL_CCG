...
with open(partial_path + r'\speeding.xml', 'r') as f_IN:
    ...
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif ('5' in dominant_a_tags.string) and ('MAN' not in dominant_a_tags.string):
                ...
                elif 'B-L\"go on\"' in dominant_a_tags.string:
                    dominant_hand_unlabeled_B_L = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_B_L)
                else:
...
"""
Lines 107-155 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
