...
with open(partial_path + r'\whitewater.xml', 'r') as f_IN:
    ...
    # Lines 44-64 is only for dominant hand gloss
    for utterance_tag in soup.find_all('UTTERANCE'):
        for dominant_track_tags in utterance_tag.find_all('TRACK', {'FID': '10000'}):
            for dominant_a_tags in dominant_track_tags.find_all('A'):
                ...
                elif 'fs-K' in dominant_a_tags.string:
                    dominant_hand_unlabeled_fs_K = dominant_a_tags.string
                    dominant_hand_unlabeled_gloss_list.append(dominant_hand_unlabeled_fs_K)
                else:
        ...
        # Lines 67-82 is only for non-dominant hand gloss
...
"""Code blocks from 86 to 94 take the gloss from both the dominant and non-dominant hand and
...
# lines 97-105 arrange the output to be in <POS> <GLOSS> ordering
...
"""
Lines 113-163 arranges the output for the file pos_gloss.txt to be <POS> <GLOSS WORD>. Any whitespace that happens 
